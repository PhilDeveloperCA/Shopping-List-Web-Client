...
const ShoppingList:React.FC<ShoppingListInterface> = ({jwt}) => {
    ...
    useEffect(()=> {
        const fetchItems = async () => {
            try {
                const response : AxiosResponse = await instance.get(`/api/items?list=${listid}`);
    ...
    const deleteItem = async (itemid :number, index:number) => {
        instance.delete(`/api/items/${itemid}`)
    ...
    const SubmitItem = async (event:any) => {
        ...
        if(itemName&&itemDescription){
            try{
                const response = await instance.post(`/api/items/${listid}`, {
                    name : itemName,
                    description : itemDescription,
                    category1 : category1,
                    category2 : category2,           
                })
                const item:Item= {...response.data[0], username:state.user};
                setItems([...items,item])
                        ...
                        <TableBody>
                            {items?.map((item,index) => {
                                ...
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell align="right"> <IconButton color="primary" onClick={(e:any) => {e.preventDefault(); deleteItem(item.id, index)}}> <DeleteIcon /> </IconButton></TableCell>
                                </TableRow> );
                            })}
                        </TableBody>
                    </Table>
